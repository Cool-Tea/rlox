...
test: build $(TEST_RES)
	@echo -e "\033[32;1mPassed: `find $(TEST_DIR)/res/ -name *.pass | wc -l`(`find $(TEST_DIR)/res/ -name *.pass -exec basename {} \;`) , \033[31;1mFailed: `find $(TEST_DIR)/res/ -name *.fail | wc -l`(`find $(TEST_DIR)/res/ -name *.fail -exec basename {} \;`)\033[0m" 
	

$(TEST_DIR)/res/%.pass: $(TEST_DIR)/%.in build
	@echo -e "\033[33;1mTesting: $*\033[0m"
	@-{\
		$(RLOX) $< > $(TEST_DIR)/res/$*.res; \
		res=$$?; \
		if [ $$res -eq 101 ]; then \
			echo -e "\033[31;1mFail to run($$res)!\033[0m"; \
		...
		if [ $$? -eq 0 ]; then \
			...
			echo -e "\033[31;1mFail! Expect: \033[0m$$expect"; \
			echo -e "\033[31;1mActual: \033[0m$$actual"; \
			touch $(TEST_DIR)/res/$*.fail; \
		fi; \
	}